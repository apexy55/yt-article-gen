...
<div class="container">
  ...
  <div class="card">
    ...
    <p class="fallback-hint">提示：若字幕获取失败，系统会自动使用内置示例字幕（xRh2sVcNXQ8）</p>

    <label for="prompt" style="margin-top:18px">💬 生成要求（可选）</label>
...
async function generate() {
  ...
  try {
    ...
    // Final render with section buttons
    articleDiv.innerHTML = buffer;
    addSectionButtons();
...
function addSectionButtons() {
  document.querySelectorAll('#article h2[data-section]').forEach(h2 => {
    ...
    btn.onclick = async () => {
      ...
      if (panel.querySelector('table')) return; // cached

      const sectionId = h2.id || h2.textContent.trim().slice(0, 20);
